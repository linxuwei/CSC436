 ...
 * Reference on:https://www.w3schools.com/js/js_json_parse.asp
 */
...
const Product = class Product {
    ...
    constructor(product){
        const proArray=product.split(',');
        this.name=proArray[0];
        this.price=Number(proArray[1]);
        this.availability=proArray[2];
    ...
    static halfOff = (products) => {
        return products.map((product) => ({name:product.name,price:(product.price)*0.5,availability:product.availability}));
    }
    ...
    static printProducts = (products) => {
        ...
        products.forEach(product =>  console.log(`Product: ${product.name}, Cost: $${product.price.toFixed(2)}, Availability: ${ product.availability !== 'In Stock' ? 'No':'Yes'}`)
        )
    }
